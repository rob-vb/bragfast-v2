...
Primary user is a seeker planning a weekend breakfast or already in a Dutch woonplaats, asking where to eat. Secondary users are adders who publish the first photo in the app, visitors who like a spot after they sign in, visitors who attach more photos from the iOS and Android app, and one owner who reviews reports and closed spots. Inferred from `SPEC.md`.

## Product Purpose

A crawlable breakfast and brunch directory for the Netherlands. Spots are the unit. App users add them by publishing the first photo. The website is the seeker directory and has no add UI. Likes are how spots climb a woonplaats board. Success is a seeker who can pick a place in a stad or dorp without treating Google stars as "best." Inferred from `SPEC.md`.
...
Live Google Places autocomplete is live when `GOOGLE_PLACES_API_KEY` is set on the Convex deployment. The app uses it as the add helper. The website does not.
...
3. Browse signed-out. Sign-in for like and passport identity. A signed-out like opens the sign-in dialog.
