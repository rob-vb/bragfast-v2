...
A woonplaats page lists breakfast spots added in the app. Empty copy is the proof on Haarlem when the board has zero spots.

## Sub-features

- `city-empty` shows Haarlem with `noSpotsYet` and `emptyBoardAppHint`, and no catalog tents.
...
- **Open Haarlem.** Run `… http GET /nl/haarlem --out artifacts/city-board/haarlem.html`. Status `200`, h1 `Haarlem`. HTML includes `Nog geen plekken in deze stad.` and `Een plek verschijnt hier wanneer iemand de eerste foto toevoegt in de app.` and does not include `Anne&Max` / `Anne&amp;Max`. Header uses `bg-berry` (class on the hero section). No `PhotoFrame` city still required in the HTML as an `<img>` of a scene behind the h1.
- **No website add.** Same file does not include `Plek toevoegen` or `Add a spot`.
