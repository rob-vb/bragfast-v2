...
- **Switch to English.** Run `… http POST /api/locale --json '{"locale":"en"}'` then `… http GET / --out artifacts/language-switch/en-home.html`. POST status `204`. GET `html lang="en"`, h1 `Breakfast and brunch spots, by city.`, submit `Search`, sign-in `Sign in`, app row `Be first to brag`. No `Cities to explore`.
